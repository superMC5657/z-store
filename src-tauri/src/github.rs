    ...
    pub fn rewrite_readme_images(raw_markdown: &str, owner: &str, repo: &str) -> String {
        ...
        let ref_replaced = MD_REF_LINK_RE.replace_all(&md_replaced, |caps: &regex::Captures| {
            ...
            let is_img = lower_url.ends_with(".png")
                ...
                || (!url.starts_with("http://") && !url.starts_with("https://") && !url.starts_with('#') && !url.starts_with("mailto:"));
            if is_img {
                let rewritten_url = Self::clean_image_url(url, owner, repo);
        ...
        let source_replaced = HTML_SOURCE_RE.replace_all(&html_replaced, |caps: &regex::Captures| {
            let before = &caps[1];
            let srcset = &caps[2];
            let after = &caps[3];
            let rewritten_srcset = srcset
                ...
                .collect::<Vec<_>>()
                .join(", ");
            format!(r#"<source {}srcset="{}"{}>"#, before, rewritten_srcset, after)
        });
    ...
    pub async fn sync_starred_repos(
        ...
        if !status.is_success() {
            ...
            if code == 401 {
                ...
                return Err(format!("未在 GitHub 上找到用户「{}」，请检查用户名拼写是否正确。", user_hint));
            } else {
                return Err(format!(
...
"#;
        ...
        assert!(rewritten.contains("https://user-images.githubusercontent.com/71636191/171661982-demo.png"));
